...
namespace Clidapos.Wpf.Views
{
    public partial class WarehouseTypePopup : Window
    {
        ...
        /// <summary>
        /// Save only ever creates a brand-new warehouse type. Once something has been
        ...
        private async void Save_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                ErrorText.Text = "Type a warehouse type name first.";
                return;
            }

            var existing = await _warehouseTypeService.GetAllAsync();
            if (existing.Any(c => c.Equals(name, StringComparison.OrdinalIgnoreCase)))
            {
                ErrorText.Text = $"A warehouse type named '{name}' already exists.";
                return;
            }
        ...
        private async void Update_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                ErrorText.Text = "Use Get Data, pick a warehouse type, then edit and Update.";
                return;
            }
            ...
            {
                ErrorText.Text = $"A warehouse type named '{newName}' already exists.";
                return;
            }

            var confirm = MessageBox.Show(
                $"Rename warehouse type '{_editing}' to '{newName}'?",
        ...
        private async void Delete_Click(object sender, RoutedEventArgs e)
        {
            if (string.IsNullOrEmpty(_editing))
            {
                ErrorText.Text = "Use Get Data, pick a warehouse type, then Delete.";
                return;
            }
